...
This page starts oonfeeWRT v0.1.8 on one host and creates the first owner account. It offers a standalone-binary path and a Docker Compose path; use only one.
...
- either Docker with Compose support or the v0.1.8 release archive;
...
Create a private working directory, download the exact v0.1.8 Compose file, and create the runtime passphrase:
...
The version command must print `v0.1.8`, and the service must report healthy.

In v0.1.8, **Workspace** groups **Dashboard**, **Devices**, **Client Devices**,
...
Then open `http://127.0.0.1:8081`. With the v0.1.8 Compose file, change
`published: "8080"` to the desired host port and keep `target: 8080`.

### The browser is on another computer

`127.0.0.1` is reachable only on the controller host. The v0.1.8 Compose file
