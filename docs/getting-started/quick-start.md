...
This page starts oonfeeWRT v0.1.7 on one host and creates the first owner account. It offers a standalone-binary path and a Docker Compose path; use only one.
...
- either Docker with Compose support or the v0.1.7 release archive;
...
Create a private working directory, download the exact v0.1.7 Compose file, and create the runtime passphrase:
...
The version command must print `v0.1.7`, and the service must report healthy.

In v0.1.7, **Workspace** groups **Dashboard**, **Devices**, **Client Devices**,
**Topology**, **Radios**, **Policy Engine**, and **Adopt a device**.
**Insights** groups **Statistics**, **Reports**, and **Alerts**.
The sidebar places **Settings**, **Accounts**, and **Logs**, in that order,
in a **Controller** group at the foot of the sidebar, followed by your compact
profile shortcut to Accounts. The group remains reachable on a short screen.
Open **Settings → Firmware** or **Settings → Integrations** for optional checks;
opening either tab only loads stored state and does not run a check.
New section bookmarks use `/settings?section=firmware` and
`/settings?section=integrations`; the old standalone URLs remain supported.
...
the same workspaces with full labels. Desktop navigation can remain collapsed
or expanded independently. See [Accounts, roles, and sessions](../operations/accounts.md).
...
Then open `http://127.0.0.1:8081`. With the v0.1.7 Compose file, change
`published: "8080"` to the desired host port and keep `target: 8080`.

### The browser is on another computer

`127.0.0.1` is reachable only on the controller host. The v0.1.7 Compose file
