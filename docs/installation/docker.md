...
The v0.1.7 container is a multi-platform Linux image containing one static controller binary, CA roots, licenses, and release material. The final image has no shell or package manager.
...
The v0.1.7 Compose file keeps loopback as the safe default and accepts
...
Install `cosign` using Sigstore's official instructions, then verify the v0.1.7 GitHub Actions identity:
...
Deployments should pin `v0.1.7` or the reported digest. The `0.1.7`, `0.1`, and `latest` aliases may resolve to the same manifest but are not immutable deployment intent.
