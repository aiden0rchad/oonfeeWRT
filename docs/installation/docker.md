...
The v0.1.8 container is a multi-platform Linux image containing one static controller binary, CA roots, licenses, and release material. The final image has no shell or package manager.

Use these artifact instructions only after the completed exact `v0.1.8` tag
workflow and GitHub release establish publication. Source preparation, a
version string, or a tag alone does not prove that the image is available.
...
<span id="_4-pin-and-start-v0-1-7"></span>

## 4. Pin and start v0.1.8
...
The v0.1.8 Compose file keeps loopback as the safe default and accepts
...
Install `cosign` using Sigstore's official instructions, then verify the v0.1.8 GitHub Actions identity:
...
Deployments should pin `v0.1.8` or the reported digest. The `0.1.8`, `0.1`, and `latest` aliases may resolve to the same manifest but are not immutable deployment intent.
