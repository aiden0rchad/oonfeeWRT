...
> **Outcome:** A checksum-verified oonfeeWRT v0.1.5 binary runs on a supported Linux or macOS host, stores private state outside the program directory, and answers only on loopback.
...
The macOS binaries are not Developer ID signed or notarized. The checksum-verified GitHub release is the authenticity path documented for v0.1.5.
