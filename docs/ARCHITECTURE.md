...
Generated by the controller at adoption and written **over SSH**, because ubus
cannot write it — see §2's bootstrap note.
...
> **The bootstrap is the one exception, and it is not optional.** Measured on a
...
>
> So the two writes adoption exists to perform go over **SSH, exactly twice in a
> device's life**: once at adoption to install the ACL file and the login, once
> at un-adoption to remove them. Everything in between — every poll, every
> apply, every read — is ubus, and every argument below still holds. This is a
> bootstrap channel, not a transport.
>
...
3. Controller, in one transaction:
   ...
   - writes `/usr/share/rpcd/acl.d/oonfeewrt.json` **over SSH** (ubus refuses
     it even to root — §2), verified by `sha256sum`
