...
| Metric | Source | Notes |
|---|---|---|
| **Which clients are connected** | ⚠️ **UNRESOLVED — do not build the Clients screen on a single source until this is settled.** | The two sources were measured **disagreeing for 131 s continuously**: `hostapd.get_clients` listed a station with `auth`/`assoc`/`authorized` all `true` while `iwinfo.assoclist` reported zero stations on the same radio. hostapd's own event log (`AP-STA-CONNECTED` at 13:54:22, `AP-STA-DISCONNECTED` at 13:59:22) brackets the whole divergence window, which points to **`iwinfo` being the source at fault**, not hostapd — the reverse of the intuitive reading. The station was idle at the time (last activity ~50 s prior), so the leading hypothesis is that `iwinfo.assoclist` drops or fails to report power-saving stations. **Re-run with a deliberately idle client to settle it.** Until then: cross-check both sources, treat a station present in *either* as connected, and drive connect/disconnect transitions from hostapd's events rather than from either snapshot. |
