...
- **SQLite** for config, inventory, clients, events, audit log. WAL mode.
  No server to run. Back up a live store with SQLite's `.backup` API (or after
  clean shutdown/checkpoint), never by copying the main file alone while WAL is
  active; the latter can omit committed schema/data. Pair every database backup
  with that controller's `keyring.json`: neither a database nor the passphrase
  can reconstruct the keyring's random data key. This is correct at home and
  small-business scale; resist Postgres until you have a reason.
...
| Database | **SQLite** (WAL) | Zero-ops and ample at this scale; live backup uses SQLite `.backup` or a clean checkpoint and is paired with `keyring.json`, not a main-file-only copy. |
