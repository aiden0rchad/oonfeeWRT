...
| Metric | Source | Notes |
|---|---|---|
| Per-client RSSI, PHY rate, TX retries, connected time | **Both, by tier — they are not substitutes.** `hostapd.<iface> get_clients` (**2.1 ms** with 2 stations) on the focused loop; `iwinfo.assoclist` (**30.3 ms**) on the slow loop and whenever a client detail view is open. `iw station dump` is **not needed at all** and should not be granted. | Measured against two real associated stations. Their byte/packet counters are **identical**, so the cheap source is trustworthy for volume. But `hostapd` **lacks** `tx.retries`, `tx.failed`, `connected_time`, `signal_avg`, `noise` and `thr` — so the TX-Retries column, SNR, and "connected for" all require `iwinfo`. Two traps: `hostapd.rate` is **100× `iwinfo.rate`** (14440000 vs 144400 kbit/s) — never mix them in one series; and `hostapd`'s per-client `airtime` is `{rx:0, tx:0}` on mwlwifi, matching the radio-level `rx_time`/`tx_time` gap. |
