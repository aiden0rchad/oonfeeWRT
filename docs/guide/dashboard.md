...
Open **Workspace → Dashboard**. The shared fleet strip summarizes the whole
current fleet, with labels and scope notes on the left and counts on the
right. Its five metrics retain the same meaning as the v0.1.6 overview cards:
...
The strip uses one surface with subtle dividers instead of five separate
card outlines. Labels stay aligned when some metrics need helper text and
others do not. At smaller widths the strip becomes two columns, then a single
column; decorative icons can disappear to make room, but values and evidence
explanations remain. A large count or long source note can expand a cell—text
is not clipped to preserve a fixed height.

<DocScreenshot
  src="dashboard-overview" :width="1600" :height="1000"
  alt="oonfeeWRT Dashboard with the compact fleet summary strip and Precision navigation"
...
<DocScreenshot
  src="dashboard-internet" :width="1600" :height="1000"
...
Open **Insights → Statistics** when you need
...
3. Select **Run speed test**. This action is the explicit,
