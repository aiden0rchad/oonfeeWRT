...
::: info New workspaces do not change network authority
Reports and Alerts add observation and notification, not automatic network
reconfiguration. AdGuard Home integration reads an explicitly configured DNS
service; it does not replace this DHCP/network editor or change client DNS.
WireGuard integration reads existing peer evidence; it does not create tunnels
or management routes. Continue to use reviewed Preview/Apply for supported
network intent. See [Integrations](./integrations.md) for those separate checks.
:::
...
v0.1.8 because port topology and safe mutation are hardware-specific.
...
mean inspection missed hardware. v0.1.8 does not create tagged VLAN
...
| Single-interface LAN with no switch ports | The layout was read successfully, but v0.1.8 cannot create a tagged VLAN attachment on it | Keep existing LAN/VLAN configuration unchanged or prepare it manually with an OpenWrt-specific recovery-tested plan, then generate a fresh Preview |
