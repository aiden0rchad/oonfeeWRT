...
description: What oonfeeWRT v0.1.5 can do, what depends on device evidence, and what is unavailable.
---
...
| Status | Meaning |
|---|---|
| **Shipped** | Present in v0.1.5 source, UI/API, and automated tests |
...
| Capability | v0.1.5 status | Important boundary |
|---|---|---|
...
| Schema-23 upgrade | **Shipped** | First v0.1.5 startup migrates schema 20 → 21 → 22 → 23 without configuring routers: management mode, policy sets, then source-relative client provenance, bounded MAC lookup indexes, and a rebuilt one-managed-Gateway uniqueness guard based on canonical functions plus the compatibility role. Existing devices become Managed; v0.1.4 cannot open schema 23, so direct rollback requires the matching pre-upgrade database, keyring, and runtime passphrase |
| Responsive light and dark controller UI | **Shipped** | PageHeader/action/status patterns are aligned across the principal screens, with desktop/mobile browser coverage in both themes; the UI defaults to dark and does not persist the choice across reloads |
...
| Application/DPI identification and flow history | **Unavailable** | v0.1.5 ships a feasibility and pilot-gate document only; it installs neither `nlbwmon` nor `netifyd`, and makes no application-identity claim |
...
| Durable per-client application usage | **Unavailable** | Requires optional accounting/DPI not shipped as a v0.1.5 controller capability; see the gated [flow-visibility feasibility review](./flows-feasibility.md) |
...
| Client block and fixed-IPv4 desired policy | **Shipped** | Both are MAC-scoped and require a stored `local` observation from the currently adopted Managed Gateway. Active intent blocks Preview while that proof is absent, including after an upgrade/restore until the next successful managed-Gateway poll; existing intent can still be cleared one client at a time. Per-client rate limiting, QoS/SQM, and application/DPI policy backends are unavailable in v0.1.5 |
...
The current controller serves REST/WebSocket routes under `/api/v1`, but v0.1.5
