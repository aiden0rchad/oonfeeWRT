...
type ApplyRequest struct {
	// OperationID is generated by the browser before it starts the request. It
	// makes a lost response recoverable without ever retrying a router write.
	OperationID string `json:"operation_id"`
	// PreviewToken binds this write to a server-built full-fleet preview. Apply
	// rejects a missing or stale token before any device write.
	PreviewToken string `json:"preview_token"`
	...
	// AcknowledgeDriverRisk is required when the selected plan knowingly
	// triggers a driver defect severe enough to take a radio down. Unlike a
	// traversal change, that failure may not be reachable to roll back.
	AcknowledgeDriverRisk bool `json:"acknowledge_driver_risk,omitempty"`
	// AcknowledgeCautions confirms plan warnings that still render and apply,
	// such as an open mesh or a wireless-uplink loop risk.
	AcknowledgeCautions bool `json:"acknowledge_cautions,omitempty"`
	// AcknowledgePartialFleet is required when DeviceIDs intentionally omits
	// adopted devices. The controller UI never sets it; scoped API clients must
	// say explicitly that they accept a temporarily inconsistent fleet.
	AcknowledgePartialFleet bool `json:"acknowledge_partial_fleet,omitempty"`
}

var (
	ErrPreviewRequired = errors.New("Preview again before applying; nothing was written")
	ErrPreviewStale    = errors.New("the preview is stale; Preview again before applying; nothing was written")
)
...
type DeviceApply struct {
	...
	// RouterOutcome preserves what the apply engine established on the device
	// when controller bookkeeping fails afterwards. In that case Outcome is
	// error while RouterOutcome can still truthfully be applied.
...
// wlanView never returns the passphrase. Key remains a write-only request field
// so create and explicit rotation keep using the same wire type.
//
...
// screen actually asks — "is this secured". A signed-in browser does not need
// reusable plaintext credentials to edit unrelated settings; an empty key on
...
func viewWLAN(w model.WLAN) wlanView {
	v := wlanView{
...
// dhcpRequest keeps field presence at the HTTP boundary. A plain dhcpView
// cannot tell an omitted boolean from an explicit false, so decoding `{}` into
// it used to turn a running DHCP server off and let prune delete its section.
// Older clients remain compatible by omitting the whole dhcp object.
...
func (v *dhcpRequest) toModel() (*model.DHCPConfig, error) {
	...
	if v.LeaseTime == nil {
		missing = append(missing, "leasetime")
	}
	if len(missing) > 0 {
		return nil, errors.New("dhcp must include enabled, start, limit and leasetime; missing " +
			strings.Join(missing, ", "))
	}
	return &model.DHCPConfig{
		Enabled: *v.Enabled, Start: *v.Start, Limit: *v.Limit,
		LeaseTime: strings.TrimSpace(*v.LeaseTime),
	}, nil
}

// networkRequest is a merge request. The TypeScript client has always exposed
// saveNetwork as Partial<SiteNetwork>; decoding that into networkView instead
// rebuilt every omitted field from its zero value. A DHCP-only update could
// therefore disable and rename the whole network before DHCP was even read.
...
func (v networkRequest) apply(n *model.Network) error {
	if v.Name != nil {
	...
	if v.CIDR != nil {
		n.CIDR = strings.TrimSpace(*v.CIDR)
	}
	if v.Zone != nil {
		n.Zone = strings.TrimSpace(*v.Zone)
	}
	if v.Enabled != nil {
		n.Enabled = *v.Enabled
	}
	if v.DHCP != nil {
		dhcp, err := v.DHCP.toModel()
		if err != nil {
			return err
		}
		n.DHCP = dhcp
...
type zonePolicyView struct {
	Name      string   `json:"name"`
	ForwardTo []string `json:"forward_to"`
	Explicit  bool     `json:"explicit"`
}

func viewZonePolicy(p model.ZonePolicy) zonePolicyView {
	forward := p.ForwardTo
	if forward == nil {
		forward = []string{}
	}
	return zonePolicyView{
		Name: p.Name, ForwardTo: forward, Explicit: p.Explicit,
	}
}
...
func (s *Server) handleSite(w http.ResponseWriter, r *http.Request) {
	...
	for _, n := range site.Networks {
		nets = append(nets, viewNetwork(n))
	}
	zones := make([]zonePolicyView, 0, len(site.Zones))
	for _, p := range site.EffectiveZonePolicies() {
		zones = append(zones, viewZonePolicy(p))
	}
...
func (s *Server) handleGetWLAN(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range site.WLANs {
		if x.ID == id {
			// Query parameters cannot turn this read endpoint into a credential
			// export. has_key carries the only state an edit form needs.
			writeJSON(w, http.StatusOK, viewWLAN(x))
			return
		}
	}
...
func (s *Server) handleSaveWLAN(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{
		"wlan": viewWLAN(m), "problems": problems,
	})
}
...
func (s *Server) handleSaveNetwork(w http.ResponseWriter, r *http.Request) {
	...
	if err := v.apply(&n); err != nil {
		writeErr(w, http.StatusBadRequest, err.Error())
		return
	}
	if v.DHCP == nil {
		// Preserve this atomically in Store.SaveNetwork. Keeping the value loaded
	...
	writeJSON(w, http.StatusOK, viewNetwork(n))
}
...
func (s *Server) handleSaveZonePolicy(w http.ResponseWriter, r *http.Request) {
	if !s.lockSiteMutation(w, r) {
		return
	}
	defer s.siteMu.Unlock()
	...
	if req.ForwardTo == nil {
		writeErr(w, http.StatusBadRequest, "forward_to must be an array")
		return
	}
	p := model.ZonePolicy{Name: name, ForwardTo: *req.ForwardTo, Explicit: true}
	if err := s.Store.SaveZonePolicy(r.Context(), &p); err != nil {
		writeErr(w, http.StatusBadRequest, err.Error())
		return
	}
	s.logSiteChange(r.Context(), "site.zone_policy_saved", map[string]any{
		"zone": p.Name, "forward_to": p.ForwardTo,
	})
	writeJSON(w, http.StatusOK, viewZonePolicy(p))
}

func (s *Server) handleDeleteZonePolicy(w http.ResponseWriter, r *http.Request) {
	if !s.lockSiteMutation(w, r) {
		return
	}
	defer s.siteMu.Unlock()
	name := r.PathValue("name")
	if err := s.Store.DeleteZonePolicy(r.Context(), name); err != nil {
		if errors.Is(err, store.ErrNotFound) {
			writeErr(w, http.StatusNotFound, "no explicit zone policy")
			return
		}
		writeErr(w, http.StatusBadRequest, err.Error())
		return
	}
	s.logSiteChange(r.Context(), "site.zone_policy_reset", map[string]any{"zone": name})
	writeJSON(w, http.StatusOK, zonePolicyView{
