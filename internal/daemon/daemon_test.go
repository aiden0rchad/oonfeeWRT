...
// A prompt that cannot be answered should say what would answer it.
//
// The commonest cause of a failed unlock in practice is not a typo: it is that
// the passphrase was generated by an automated run and written to a file, so
// nobody ever knew it. The underlying error — "wrong passphrase, or the keyring
// is corrupt" — is accurate and answers none of "what do I do now".
//
// Tested on the message rather than through Open, because the branch only
// exists when a human was prompted at a real terminal, which a test cannot
// stage. Without a terminal the daemon fails earlier and more helpfully.
func TestUnlockHintNamesThePassphraseFile(t *testing.T) {
	base := errors.New("secrets: cannot unwrap the key")

	got := unlockHint(Config{}, "/data/keyring.json", base)
	if !strings.Contains(got.Error(), "-passphrase-file") {
		t.Errorf("a failed unlock did not name the flag that would fix it: %v", got)
	}
	if !strings.Contains(got.Error(), "/data/keyring.json") {
		t.Errorf("the hint does not say which keyring: %v", got)
	}
	// The original failure has to survive: a caller matching on it must keep
	// working, and the hint is an addition rather than a replacement.
	if !errors.Is(got, base) {
		t.Error("the hint replaced the underlying error instead of wrapping it")
	}
}

// The other half, which is what makes the first half safe to trust: someone who
// already passed the flag has a different problem, and telling them to use the
// flag they just used is noise dressed as help.
func TestUnlockHintStaysQuietWhenAFileWasUsed(t *testing.T) {
	base := errors.New("secrets: cannot unwrap the key")

	got := unlockHint(Config{PassphraseFile: "/run/pass"}, "/data/keyring.json", base)
	if strings.Contains(got.Error(), "-passphrase-file") {
		t.Errorf("told an operator to use the flag they already used: %v", got)
	}
	if got != base {
		t.Errorf("wrapped an error it had nothing to add to: %v", got)
	}
}
