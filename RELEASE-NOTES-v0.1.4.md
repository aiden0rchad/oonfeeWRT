...
Publication is complete only when the `v0.1.4` tag workflow succeeds. Verify
...
- Repeated OpenWrt "no usable IPv6 default route" messages are condensed into
  one condition per router-log evidence epoch. The UI names the affected router
  and links to the relevant network setting. This bounds controller database
  rows; it does not suppress or delete messages in the router's own log.
...
- Unchanged topology links no longer churn history, closed-history lookup is
  indexed, and the browser avoids overlapping or abandoned topology requests.
  These changes reduce the CPU and database work reported in issue #20.
...
coverage; final validation on each reporter's hardware remains pending.
...
v0.1.4 migrates the controller database from schema 19 to schema 20. The migration adds a
closed-topology history index and normalizes old development-era
